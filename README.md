...
面向**初中数学**的「讲给同伴听」**Android App**（平板优先）。学生通过手写 + 语音向多个 AI 角色讲题，系统追踪掌握度；家长端查看弱项与学习回放。

> 工程仓库仍沿用 [AI-Feynman](https://github.com/sunimo-pku/AI-Feynman) 代号；用户可见品牌名为 **我讲你听**。
...
| 老师视频可选增强 | 题库题目可配置 `answerVideo.asset` 或 `answerVideo.url`，讲题页会展示「老师解答视频」入口；视频资源放在 `main/mobile/assets/videos/answers/` 或使用 HTTPS 地址 |
| 讲题广场 | 学生完成讲题后可把录音、白板笔迹和同伴讨论合成为 MP4，并附一句描述发布；卡片显示用户名、首字母头像与段位；老师讲解弹窗下方按题展示同学讲法（点赞数倒序），底部「讲题广场」可浏览、点赞、评论 |
...
- WS / 麦克风 / 录音库任一失败时左下角 orb 切到「重新连接 / 再试一次」，
  **白板内容不丢、本地进度 / 回顾仓库不被擦除**；讲题主路径仅走实时 WS，
  不再提供「用文字提交」兜底入口。
...
- 可解释长期学习画像：`GET /learning/profile-insights` 与
  ...
  「今天建议练」卡片均共用同一画像出口。
- **小节讲题档案（追问专用）**：与全册规划画像分离；`GET /learning/section-profile`
  聚合本节 Review 错因 / 同伴追问 / 未完成会话；实时讲题 `session_start` 携带
  `knowledgePointId` + `knowledgePointStars`，登录用户自动注入同伴 / 李老师 Prompt
  的「本节讲题档案」背景（匿名仅拼星级行）。
